...
public:
  ...
  void print(
    ...
    if (topoSort) {
      printGraphItem("_", "_", nullptr, vertexOffset);
      std::cout << "\n";
      printGraphItem("", "", topoSort, vertexOffset);
    }
    else
      printGraphItem("", "", nullptr, vertexOffset);
  ...
  virtual T weight(T vFrom, T vTo) const override {
    if (vFrom >= matrixNeibor.size() || vTo >= matrixNeibor.size())
      return 0;
    VectorArray<T> r = matrixNeibor.get(vFrom);
    return r.get(vTo);
  ...
  virtual void printEdges(
    ...
    printGraphItem("", "", nullptr, vertexOffset, &v);
...
private:
  ...
  void printGraphItem(
    ...
    for (T row = 0; row < vertexCount(); ++row) {
      std::cout << "\"" << prefix << (row + vertexOffset) << postfix << "\"\n";
    }
    ...
    if (topoSort) {
      for (T i = 0; i < topoSort->size(); ++i) {
        ...
        for (T j = 0; j < sub.size(); ++j) {
          std::cout << "\"" << prefix << (sub.get(j) + vertexOffset) << postfix << "\"\n";
        }
        std::cout << "\t}\n";
      }
    }
      ...
      for (T col = startCol; col < r.size(); ++col) {
        if (bWeight) {
          if (r.get(col) != 0) {
            std::cout
            << "\""
            << prefix << (row + vertexOffset) << postfix
            << edgeView
            << prefix << (col + vertexOffset) << postfix
            << "\" [label=\""
            << r.get(col)
            << "\"";
